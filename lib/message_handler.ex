...
alias ChatServerBRB.State
  ...
  def handle_message(:initial, message, state) do
    ...
    {put_in(state.brb_messages[{message.initiator_pid, message.round_identifier}], current_round), echo_message}
  end
  ...
  def handle_message(:echo, message, state) do
    ...
    if length(current_round.echos_received) >= 2 * state.num_byzantine_nodes + 1 do
      ...
      {put_in(state.brb_messages[{message.initiator_pid, message.round_identifier}], current_round), echo_message ++ ready_message}
    else
      {state, []}
    end
  end
